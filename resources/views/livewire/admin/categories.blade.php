<div x-data="{ showModal: false, action: '', categoryId: null }" x-on:modal-close.window="showModal = false">
    ...
    <div class="w-full overflow-hidden rounded-lg shadow-xs">
        ...
        <form wire:submit.prevent="storeCategory" method="POST" class="flex justify-around ml-4 mt-2 flex-1 lg:mr-32">
            @csrf
                            ...
                            <td class="px-4 py-3">
                                <div class="flex items-center space-x-4 text-sm">
                                    <button wire:click="edit({{ $category->id }})"
                                        @click="action = 'edit'; categoryId = {{ $category->id }}; showModal = true"
                                    ...
                                    <button
                                        @click="action = 'delete'; $wire.set('deleteId', {{ $category->id }}); categoryId = {{ $category->id }}; showModal = true"
                                        class="flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-purple-600 rounded-lg dark:text-gray-400 focus:outline-none focus:shadow-outline-gray"
                                        aria-label="Delete">
                                        <svg class="w-5 h-5" aria-hidden="true" fill="currentColor" viewBox="0 0 20 20">
                ...
                <template x-if="action === 'edit'">
                    <div>
                        ...
                        <div class="px-4 py-3 mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800">
                            <form method="POST" wire:submit.prevent="updateCategory" id="update_category">
                                @csrf
                ...
                <template x-if="action === 'delete'">
                    <div>
                        ...
                        <p class="text-sm text-gray-700 dark:text-gray-400">
                            <strong class="text-red-600"> Danger: This category would delete along side all products
                                under it!</strong><br>
                            Click ok to confirm
                        </p>
                        ...
                        <footer
                            ...
                            <button @click="$wire.deleteCategory; showModal = false"
                                class="w-full px-5 py-3 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-purple-600 border border-transparent rounded-lg sm:w-auto sm:px-4 sm:py-2 active:bg-purple-600 hover:bg-purple-700 focus:outline-none focus:shadow-outline-purple">
                                Ok
                            </button>
